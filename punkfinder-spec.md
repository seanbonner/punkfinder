...
- **Primary liveness signal — whole-wallet last outbound tx.** "Is anyone home" = has the owner *signed* a transaction recently, of ANY kind. Punk activity is irrelevant: a punk sitting still in a wallet that's active elsewhere is **held, not lost**. Source this from a whole-chain source — Blockscout (`GET https://eth.blockscout.com/api/v2/addresses/{addr}/transactions?filter=from`, newest = last signed tx; keyless, CORS-enabled) or Etherscan/Alchemy. Only outbound/owner-signed txs count; incoming transfers (airdrops, spam) are NOT liveness.
- **Do NOT use the indexer's `lastActiveAt` for liveness — it is punk-scoped.** `GET /accounts/stats` `lastActiveAt` (from `accounts.last_interaction_at`) only reflects tx-from to *punk* contracts the indexer tracks, so it returns null/stale for a wallet active outside punks. Confirmed live: an EOA with a real non-punk tx (2025-05-13) returns `lastActiveAt: null` — reading that as "no activity → lost" is wrong. (An earlier draft of this spec incorrectly claimed `lastActiveAt` was whole-wallet; it is not.) The `/accounts/stats` spend/earn aggregates are likewise punk-scoped.
- **Acquisition ("held since"):** the most recent ownership event (assign/transfer/sale) for the punk in that version, from the indexer events — useful context for a held punk, separate from wallet liveness.
